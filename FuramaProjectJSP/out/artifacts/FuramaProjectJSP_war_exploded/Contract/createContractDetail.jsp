        ...
        <div class="row">
            ...
            <div class="col-11">
                ...
                <form action="/contractServlet" method="get">
                    <input type="hidden" name="action" value="confirmCreateContractDetail" >



                    <div class="form-group">
                        <label for="">Mã hợp đồng</label>
                        <select name="idHopDong">
                            <c:forEach var="contract" items="${contractList}">
                                <option value="${contract.id}" ><c:out value="${contract.id}"></c:out></option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="">Tên dịch vụ đi kèm</label>
                        <select name="idDichVuDiKem">
                            <c:forEach var="serviceAttach" items="${serviceAttachList}">
                                <option value="${serviceAttach.id}" ><c:out value="${serviceAttach.tenDichVuDiKem}"></c:out></option>
                            </c:forEach>
                        </select>
                    </div>
                    <div class="form-group">
